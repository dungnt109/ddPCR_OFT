...
\fancypagestyle{plain}{%
  ...
  \fancyhf{}%
  \fancyfoot[C]{\footnotesize Page \thepage\, of\, \getpagerefnumber{docendlabel} `r params$sid` `r params$mid` Generated at `r generated_date` by ddPCR OFT Pipeline `r pipeline_version`}
...
\fancypagestyle{lastpage}{%
  \renewcommand{\headrulewidth}{0pt}%
  \fancyhf{}%
  \fancyfoot[C]{\footnotesize `r params$sid` `r params$mid` Generated at `r generated_date` by ddPCR OFT Pipeline `r pipeline_version`}
  \setlength\footskip{0pt}
}
...
get_info <- function(multiwell.results) {
	tab <- t(sapply(multiwell.results, function(results) {
		...
		concentration <- round(results$concentration, digits=5)
...
get_merged_info <- function(multiwell.results, sample.names=NA) {
	...
	concentration <- round(concentration[unique.names], digits=5)
...
if (min.dx.marker.con.perwell <= 0){
        min.dx.marker.con.perwell.status <- "\\textcolor{red}{FAIL}"
} else if (min.dx.marker.con.perwell >= 10) {
	min.dx.marker.con.perwell.status <- "\\textcolor{darkgreen}{PASS}"
} else {
	min.dx.marker.con.perwell.status <- "\\textcolor{orange}{ALERT}"
}
...
hl60.marker.con.perwell <- marker.merged.info[2, "concentration"]
hl60.marker.con.perwell.formatted <- sprintf("%#.3f", abs(hl60.marker.con.perwell))

if (hl60.marker.con.perwell <= 0){
	
	hl60.marker.con.perwell.status <- "\\textcolor{darkgreen}{PASS}"

} else if (hl60.marker.con.perwell > 0 && hl60.marker.con.perwell <= 0.1){
	
	hl60.marker.con.perwell.status <- "\\textcolor{orange}{ALERT}" 

} else{
	
	hl60.marker.con.perwell.status <- "\\textcolor{red}{FAIL}"
}
...
max.h2o.marker.con.perwell.formatted <- sprintf("%#.3f", abs(max.h2o.marker.con.perwell))





if (max.h2o.marker.con.perwell <= 0){

	max.h2o.marker.con.perwell.status <- "\\textcolor{darkgreen}{PASS}"

} else if (max.h2o.marker.con.perwell > 0 && max.h2o.marker.con.perwell <= 0.1 ) {

	max.h2o.marker.con.perwell.status <- "\\textcolor{orange}{ALERT}" 
...
if (min.hl60.gus.con.perwell >= 85 && min.hl60.gus.con.perwell <= 185) {
	min.hl60.gus.con.perwell.status <- "\\textcolor{darkgreen}{PASS}"
} else if (min.hl60.gus.con.perwell < 60 || min.hl60.gus.con.perwell > 210){
    min.hl60.gus.con.perwell.status <- "\\textcolor{red}{FAIL}"
} else {
	min.hl60.gus.con.perwell.status <- "\\textcolor{orange}{ALERT}"
}

### NTC <= 2
h2o.gus.info.perwell <- get_info(c(params$h2o.gus))
max.h2o.gus.con.perwell <- max(h2o.gus.info.perwell[, "concentration"])
max.h2o.gus.con.perwell.formatted <- sprintf("%#.3f", abs(max.h2o.gus.con.perwell))

if (max.h2o.gus.con.perwell <= 0) {

	max.h2o.gus.con.perwell.status <- "\\textcolor{darkgreen}{PASS}"

} else if (max.h2o.gus.con.perwell > 0 && max.h2o.gus.con.perwell <=  0.1) {

	max.h2o.gus.con.perwell.status <- "\\textcolor{orange}{ALERT}"
...
if (min.events.per.well >= 8000 && 
    min.fu.gus.conc.perwell >= 1000 && 
    min.dx.marker.con.perwell >= 10 &&
    hl60.marker.con.perwell <= 0 &&
    max.h2o.marker.con.perwell <= 0 && 
    min.hl60.gus.con.perwell >= 85 && min.hl60.gus.con.perwell <= 185 &&
    max.h2o.gus.con.perwell <= 0 )
{

	final.qc.call <- "\\textbf{\\textcolor{darkgreen}{PASS}}"

} else if (min.events.per.well < 8000 ||
	           min.dx.marker.con.perwell <= 0 ||
	       hl60.marker.con.perwell > 0.1 ||
		   max.h2o.marker.con.perwell > 0.1 ||
		   min.hl60.gus.con.perwell < 60 || min.hl60.gus.con.perwell > 210 ||
		   max.h2o.gus.con.perwell > 0.1) 
{

	final.qc.call <- "\\textbf{\\textcolor{red}{FAIL}}"

} else {

	final.qc.call <- "\\textbf{\\textcolor{orange}{ALERT}}"
}
...
if (is_manual_threshold){
	if (is_single_manual_threshold){
		lineNum <- lineNum + 1
		writeLines(paste(lineNum, ". Using OFT assay's intensities threshold ", manual_threshold, sep=""))
	} else {
		lineNum <- lineNum + 1
		writeLines(paste(lineNum, ". Using OFT assay's intensities threshold - individual samples",  sep=""))
...
if (is.na(dx.baseline)) {
	...
	lineNum <- lineNum + 1
	writeLines(paste(lineNum, ". Using in-plate OFT/GUSB ratio at Dx.", sep=""))
} else {
	lineNum <- lineNum + 1
	writeLines(paste(lineNum, ". Using preset OFT/GUSB ratio at Dx ", dx.baseline, sep=""))
}
...
if (params$runType == "absolute") {
			...
			if (absolute.oft < 1E-5) {
			    callForReporting <- "NEG"
			} else {
				callForReporting <- oft.call 
			}
			...
			names(oft.output.values) <- c("FU OFT (copies/uL)", 
			                              ...
			                              "OFT/GUSB baseline in HL60", 
			                              "Total GUSB Copy Numbers",
			                              "", 
...
```{r child="signature_table.Rmd"}



```{r, echo=FALSE, warning = FALSE}

source("create_run_records.R", local=TRUE)
