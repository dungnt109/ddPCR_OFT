...
\fancypagestyle{plain}{%
  ...
  \fancyhf{}%
  \fancyfoot[C]{\footnotesize Page \thepage\, of\, \pageref*{LastPage} `r params$sid` `r params$mid` Generated at `r params$date` by ddPCR OFT Pipeline Version 1.1, 03 Apr 2023}
...
qc.names <- c("Min events per well ($\\ge\\!\\!$ 8000)", "Min FU GUS conc. (copies/uL; >5000, GOOD; 1000-5000; ACCEPTABLE; <1000 POOR)", "Assay controls for marker", "\\hspace{0.5cm}Positive (Dx marker conc., $\\ge\\!\\!$ 10 copies/uL)", "\\hspace{0.5cm}NTC (H2O marker conc., $\\le\\!\\!$ 0.3 copies/uL)", "Assay Controls for GUS", "\\hspace{0.5cm}Positive (HL60 GUS conc., $\\ge\\!\\!$ 30 copies/uL)", "\\hspace{0.5cm}NTC (H2O GUS conc., $\\le\\!\\!$ 0.1 copies/uL)", "\\textbf{Overall QC result}")

qc.table <- cbind(QC.criteria, QC.values, QC.status)

colnames (qc.table) <- c ("QC criteria limit", "QC values", "QC status")
...
\hline   
1. Minimum Total Events per well &     $\ge$8000 total events        &  `r format(min.events.per.well, scientific = FALSE)`  &  `r min.events.per.well.status` \\
...
if (hl60.marker.info[1, "positive"] == 0) {
	...
	lineNum <- lineNum + 1
	writeLines(paste(lineNum, ". No positive event of marker observed in HL60 wells. 1 positive event assumed.", sep=""))
}
