...
final.qc.call <- c("\\textbf{\\textcolor{red}{WARNING}}","\\textbf{\\textcolor{darkgreen}{PASS}}")[(min.events.per.well.status == "\\textcolor{darkgreen}{PASS}" && min.fu.gus.conc.perwell.status != "\\textcolor{red}{POOR}" && max.h2o.marker.con.perwell.status == "\\textcolor{darkgreen}{PASS}" && min.hl60.gus.con.perwell.status == "\\textcolor{darkgreen}{PASS}" && max.h2o.gus.con.perwell.status == "\\textcolor{darkgreen}{PASS}") + 1]
...
5. QC concentration (copies/uL) & & `r qc.concentration` & \\
...
if (params$runType == "relative") {
	...
	if (is.na(dx.baseline)) {
		...
		lineNum <- lineNum + 1
		writeLines(paste(lineNum, ". Using calculated OFT/GUSB baseline ratio in Dx.", sep=""))
	} else {
		lineNum <- lineNum + 1
		writeLines(paste(lineNum, ". Using preset OFT/GUSB baseline ratio in Dx ", dx.baseline, "." , sep=""))
	}
...
if (params$manual_hl60_baseline == TRUE) {
	...
	lineNum <- lineNum + 1
	writeLines(paste(lineNum, ". Using preset OFT/GUSB baseline ratio in HL60.", sep=""))

} else {
	
	lineNum <- lineNum + 1
	writeLines(paste(lineNum, ". Using calculated OFT/GUSB baseline ratio in HL60.", sep=""))
}
